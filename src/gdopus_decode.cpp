...
Error Opus::decode(const PackedByteArray &p_data, const Ref<godot::AudioStreamWAV> &p_audio) const {
	...
	p_audio->set_data(pcm);
	p_audio->set_format(AudioStreamWAV::FORMAT_16_BITS);
	p_audio->set_loop_begin(0);
	p_audio->set_loop_end(0);
	p_audio->set_loop_mode(AudioStreamWAV::LOOP_DISABLED);
	p_audio->set_mix_rate(48000);
	p_audio->set_stereo(is_stereo);
